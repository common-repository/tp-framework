	...
	class WP_Import extends WP_Importer {
		...
		function process_terms() {
			...
			foreach ( $this->terms as $term ) {
				...
				/**
				 * Import Woocommerce Attribute
				 * @since Tpfw Importer 1.0.0
				 */
				...
				if ( !is_wp_error( $id ) ) {
					...
					if ( isset( $term['term_id'] ) ) {
						...
						// Add woocommerce attribute termmeta data
						if ( !empty( $termmeta ) && is_array( $termmeta ) ) {
							foreach ( $termmeta as $meta ) {
								add_term_meta( $id['term_id'], $meta['key'], $meta['value'], false );
							}
						}
					}
				} else {
		...
		function process_posts() {
			...
			foreach ( $this->posts as $post ) {
				...
				// Break if filter hook 'wp_import_post_data_raw' return null.
				if ( empty( $post ) ) {
					continue;
				}
		...
		function fetch_remote_file( $url, $post ) {
			...
			$headers = tp_importer_get_http( $url, $upload['file'] );
		...
		/**
		 * Added to http_request_timeout filter to force timeout at 300 seconds during import
		 * @return int 300
		 */
