...
class Tpfw_Import_Downloader {
	...
	public function download_file( $url, $filename ) {
		...
		return Tpfw_Import_Helpers::write_to_file( $content, $this->download_directory_path . $filename );
	}
	...
	private function get_content_from_url( $url ) {
		...
		if ( empty( $url ) ) {
			return new WP_Error(
				'missing_url',
				__( 'Missing URL for downloading a file!', 'tp-framework' )
			);
		}
		...
		$response = wp_remote_get(
			$url,
			array( 'timeout' => apply_filters( 'tpfw_importer_timeout_import_file', 20 ) )
		);
		...
		if ( is_wp_error( $response ) || 200 !== $response['response']['code'] ) {
			...
			return new WP_Error(
				'download_error',
				sprintf(
					__( 'An error occurred while fetching file from: %1$s%2$s%3$s!%4$sReason: %5$s - %6$s.', 'tp-framework' ),
					'<strong>',
					$url,
					'</strong>',
					'<br>',
				...
				apply_filters( 'tpfw_importer_after_file_fetching_error', '' )
			);
		}
	...
	public function set_download_directory_path( $download_directory_path ) {
		if ( file_exists( $download_directory_path ) ) {
			...
			$this->download_directory_path = apply_filters( 'tpfw_importer_upload_file_path', trailingslashit( $upload_dir['path'] ) );
		}
	}
}
