...
//            },
            load: function (query, callback) {
                ...
                if (query.length < parseInt(min_length) || !type) {
                    return callback();
                }
     ...
     * @since 1.0.4
     */
    $.fn.tpfwUpload = function () {

        var $field = $(this);

        $field.on('change', '.tpfw_value', function (e) {
            var $input = $(this);
            var $tr = $input.closest('.tpfw-field').find('tbody tr');
            var arr = [];
            $tr.each(function () {
                var $this = $(this);
                arr.push({
                    url: $this.find('[data-name="url"]').val(),
                    name: $this.find('[data-name="name"]').val(),
                    hash: $this.find('[data-name="hash"]').val()
                });
            });

            if (arr.length == 1 && arr[0].url == '') {
                $input.val('');
            } else {
        ...
        $field.on('change', '[data-name="name"]', function (e) {
            $(this).closest('.tpfw-field').find('.tpfw_value').change();
            e.preventDefault();
        });

        $field.on('click', 'table a.delete', function (e) {

            var $this = $(this);
            var $tr = $this.closest('.tpfw-field').find('tbody tr');
            var $input = $this.closest('.tpfw-field').find('.tpfw_value');

            if ($tr.length === 1) {
                $this.closest('tr').find('input').attr('value', '');
            } else {
        ...
        $field.on('click', 'table a.insert', function (e) {
        ...
        $field.find('tbody').sortable({
            ...
            opacity: 0.65,
            stop: function (e, ui) {
                $(e.target).closest('.tpfw-field').find('.tpfw_value').change();
            }
        });
        ...
        $(document.body).on('click', '.tpfw-upload .upload_file', function (event) {
            var $el = $(this);
            var $container = $el.closest('.tpfw-field');
            ...
            if (file_frames[$container.attr('id')]) {
                file_frames[$container.attr('id')].open();
                return;
            }

            // Create the media frame.
            file_frames[$container.attr('id')] = wp.media.frames.tpfw_file = wp.media({
                // Set the title of the modal.
                title: $el.data('choose'),
                library: {
                    type: $container.data('mime_types')
                },
                button: {
                    text: $el.data('update')
                },
            ...
            file_frames[$container.attr('id')].on('select', function () {

                var file_path = '';
                var selection = file_frames[$container.attr('id')].state().get('selection');
                selection.map(function (attachment) {

                    attachment = attachment.toJSON();

                    var types = $container.data('mime_types');

                    if ($.trim(types) != '' && types.indexOf(attachment.mime) !== -1) {

                        if (attachment.url) {

                            file_path = attachment.url;
                            file_path_field.val(file_path).change();
                            $container.find('.tpfw_value').change();

                        }
                    } else {
                        /**
                         * Prevent insert if invalid mimetype
                         */
                        alert(tpfw_var.upload_invalid_mime);
                    }
    ...
    $.fn.tpfwTextfields = function () {
        ...
        var update_value = function (e, val) {
            var value = [];
            var $parent = $(e.target).closest('.tpfw-textfield');
            var $items = $parent.find('li');
            var $input = $parent.find('.tpfw_value');
        ...
        $el.on('click', '.addnew', function (e) {
            var $this = $(this);
            var $list = $this.closest('.tpfw-textfield').find('ul');
    ...
    /**
     * Get object data in element
     * Replace data-get_value  = data-getValue
     * @since 1.0.14
     */
    $.fn.tpfwData = function () {
