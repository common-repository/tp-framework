	...
	class Tpfw_Customize_Field {
		...
		public function __construct( $wp_customize, $args = array() ) {
			...
			if ( isset( $args['dependency'] ) ) {
				...
				if ( !isset( $tpfw_customizer_dependency[$args['name']] ) ) {
					$tpfw_customizer_dependency[$args['name']] = array();
				}

				$tpfw_customizer_dependency[$args['name']] = $args['dependency'];
			}
		}
...
if ( !class_exists( 'Tpfw_Customize_Panel' ) ) {
	...
	class Tpfw_Customize_Panel {
		 ...
		 * Add section
		 * 
		 * @param Tpfw_Customize_Section|array $args Section Class or array settings
		 */
