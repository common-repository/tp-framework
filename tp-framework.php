...
final class TPFW {
	...
	public function termbox_init( $screen ) {
		if ( $screen->base == 'edit-tags' || $screen->base == 'term' ) {
			do_action( 'tpfw_termbox_init', $screen );
		}
	}
	...
	public function termbox_ajax_init() {
		...
		if ( isset( $_POST['screen'], $_POST['taxonomy'], $_POST['post_type'], $_POST['action'] ) ) {
			...
			do_action( 'tpfw_termbox_init', (object) $screen );
		}
	}
	...
	public function plugin_row_meta( $links, $file ) {
		if ( TPFW_PLUGIN_BASENAME === $file ) {
			$row_meta = array(
				'page' => '<a href="https://docs.themespond.com/tp-framework" aria-label="' . esc_attr__( 'View Document', 'tp-framework' ) . '">' . esc_html__( 'Documents', 'tp-framework' ) . '</a>',
			);
